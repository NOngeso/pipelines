#!/usr/bin/python3 
# description : mcscanx output parser 
# usage       : python3 [thisfile] [mcscanout.collinearity.kaks]
# need to change the variables such as targetMin, targetMax, modal_Ks and ,importantly, constant 
# targetMin : minimum range of Ks value
# targetMax : maximum range of Ks value
# modal_Ks  : selected modal Ks value (Peak value)
...
for strSB in dicSB2cls:
	cSynt = dicSB2cls[strSB]
	cSynt.calc_medianKs()
	print(cSynt.fmedianKs,file=Outfile_R)
...
target_list.sort()
print('Ks:',target_list[0],target_list[-1],numpy.average(target_list),numpy.std(target_list),modal_Ks,file=Outfile_target)
print('MYA:',get_div_time(target_list[0]),get_div_time(target_list[-1]),get_div_time(numpy.average(target_list)),get_div_time(numpy.std(target_list)),get_div_time(modal_Ks),file=Outfile_target)
